  ...
  if (!post) {
    return (
      <div className="min-h-screen pt-44 flex flex-col items-center justify-center bg-white">
        ...
        <Link to="/blog" className="text-orange-600 font-bold text-[13px] hover:underline flex items-center gap-2 uppercase tracking-widest">
  ...
  return (
    <div className="bg-white min-h-screen pt-32 pb-24 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-8">
        {/* 1. Article Header */}
        <div className="mb-12">
          <Link to="/blog" className="inline-flex items-center gap-2 text-[11px] font-bold text-gray-400 hover:text-orange-600 transition-colors uppercase tracking-widest mb-12 group">
            <ChevronLeft size={14} className="group-hover:-translate-x-0.5 transition-transform" /> Back to journal
          </Link>
          
          <div className="flex items-center gap-4 mb-8">
            <span className="text-gray-400 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
              <Clock size={12} className="text-orange-600" /> {post.readTime} read
            </span>
          </div>

          <h1 className="text-3xl md:text-5xl font-black text-gray-950 tracking-tight leading-[1.1] mb-12">
            {post.title}
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-between gap-8 border-y border-gray-100 py-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] font-medium text-gray-400 flex items-center gap-2 uppercase tracking-wider">
                  <Calendar size={10} className="text-orange-600" /> {post.date}
                </p>
              </div>
            </div>
            <button 
              onClick={handleShare}
              className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:text-orange-600 hover:bg-orange-50 transition-all cursor-pointer border border-transparent hover:border-orange-100"
            >
              <Share2 size={18} />
            </button>
          </div>
        ...
        <div className="mb-16">
          <div className="rounded-[32px] overflow-hidden bg-gray-100 shadow-xl border border-gray-100 max-w-3xl mx-auto">
            <img loading="lazy" src={post.image} alt={post.title} className="w-full h-auto aspect-video object-cover" />
          </div>
        </div>
          ...
          <div className="mt-20 pt-16 border-t border-gray-100 text-center">
            <div className="bg-gray-50 rounded-[40px] p-10 md:p-16 relative overflow-hidden">
              <div className="relative z-10">
                <div className="w-14 h-14 rounded-2xl bg-white shadow-xl flex items-center justify-center text-orange-600 mx-auto mb-8">
                  <Sparkles size={28} />
                </div>
                <h3 className="text-2xl md:text-3xl font-black text-gray-950 mb-6 tracking-tight">Did this article help you?</h3>
                <p className="text-gray-500 mb-10 max-w-md mx-auto text-sm md:text-base font-medium">Read more helpful tips and guides about keeping your home appliances in perfect condition.</p>
                <Link to="/blog" className="inline-flex items-center gap-2 px-10 py-5 bg-gray-950 text-white rounded-xl font-bold text-[13px] hover:bg-orange-600 transition-all shadow-xl active:scale-95 group">
                  <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" /> Back to all posts
                </Link>
              </div>
